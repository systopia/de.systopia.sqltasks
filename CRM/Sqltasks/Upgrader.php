...
class CRM_Sqltasks_Upgrader extends CRM_Extension_Upgrader_Base {
  ...
  public function install() {
    ...
    $this->addConstraintToExecutionSqltaskTable();
  ...
  /**
   * Adds CONSTRAINTs to civicrm_sqltasks_execution
   *
   * Issues: autogenerated civicrm_sqltasks_execution table will be created before civicrm_sqltasks, and it makes CONSTRAINT error!!!
   * To fix it use this method to add CONSTRAINTs after creating all tables
   * TODO: generate all installation of tables by civix
   *
   * @return void
   */
  public function addConstraintToExecutionSqltaskTable() {
    $updateTableSql = '
      ALTER TABLE `civicrm_sqltasks_execution`
      ADD CONSTRAINT FK_civicrm_sqltasks_execution_sqltask_id FOREIGN KEY (`sqltask_id`) REFERENCES `civicrm_sqltasks`(`id`) ON DELETE SET NULL,
      ADD CONSTRAINT FK_civicrm_sqltasks_execution_created_id FOREIGN KEY (`created_id`) REFERENCES `civicrm_contact`(`id`) ON DELETE SET NULL
    ';

    CRM_Core_DAO::executeQuery($updateTableSql);
  }
